# Lab Report

Intro: This lab focuses on GNSS data collection, raw GNSS data process to observe useful information including satellites per epoch, C/N0, GDOP, flags, positioning output, and error metrics.

Link to lab report:[AAE4203 Lab1 25016683D.pdf](https://github.com/user-attachments/files/23435872/AAE4203.Lab1.25016683D.pdf)


# How It's Made

Tech used: 

u-center, MATLAB, Python, RTKLIB, Google Earth

Proceduces:

1.Connect the GNSS Receiver and Antenna to Laptop

2.Logging data by U-center.

3.Convert the raw data to u-box UBX format

4.Open the ubx format data on u-center and plot the graphs

5.Present the output on RTKplot and Google Earth

6.Process GNSS data with Python scripts and evaluation positioning error

# Learned

Operate GNSS data with u-center

Process raw GNSS data with RTKLIB

Process GNSS data with Python scripts

Evaluation positioning error

Understanding SPP model and least squares

# Reference

https://en.wikipedia.org/wiki/Least_squares

AAE 4203 lab tutorial
